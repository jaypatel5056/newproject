...
import Link from "next/link";
import SinglePost from "../../components/blogsection/getCategoryPosts";
...
export async function getStaticPaths() {
  ...
  const posts = posts1.data;
  const paths = posts.map((post) => ({
    params: { blog4: post.id.toString() },
  }));
...
export async function getStaticProps(context) {
  ...
  try {
    ...
    const response = await axios.get(
      `https://testapivai.000webhostapp.com/wp-json/wp/v2/posts/${params.blog4}`,
      {
        headers: {
          Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOjEsIm5hbWUiOiJhZG1pbiIsImlhdCI6MTY3ODM0MzYzNiwiZXhwIjoxODM2MDIzNjM2fQ.5f6q0jPv6NlGYoxlwuM-GOS-mS1A8AQ3OjIuHMMZ9fE`,
        },
    ...
    const categoryData = await axios.get(
      `https://testapivai.000webhostapp.com/wp-json/wp/v2/posts?categories=${id}`,
      {
        headers: {
          Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOjEsIm5hbWUiOiJhZG1pbiIsImlhdCI6MTY3ODM0MzYzNiwiZXhwIjoxODM2MDIzNjM2fQ.5f6q0jPv6NlGYoxlwuM-GOS-mS1A8AQ3OjIuHMMZ9fE`,
        },
